<?php
    ...
    function process($newimage){
        ...
        $date = date("Y-m-d");
        $sql = "UPDATE posts SET ptitle = '$title',pdesc = '$desc',date = '$date',pvideo = '$videolink',pimage = '$newimage',pcat = $category WHERE pid = $pid;";
        if($old_cat != $category){
    ...
    function NULL($img){
      ...
      $pid = mysqli_real_escape_string($conn,$_POST['pid']);
      $date = date("Y-m-d");
      $sql = "UPDATE posts SET ptitle = '$title',pdesc = '$desc',date = '$date',pvideo = '$videolink',pcat = $category WHERE pid = $pid;";
      if($old_cat != $category){
          $sql .= "UPDATE category SET post_under_cat = post_under_cat - 1 WHERE cid = $old_cat;UPDATE category SET post_under_cat = post_under_cat + 1 WHERE cid = $category;";
      }
      $ans = mysqli_multi_query($conn,$sql);

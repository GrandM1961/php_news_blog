<?php
    ...
    if(!empty($img)){
        ...
        $date = date("Y-m-d");
        $sql = "UPDATE posts SET ptitle = '$title',pdesc = '$desc',pvideo = '$videolink',date = '$date',pcat = $category WHERE pid = $pid;";
        if($old_cat != $category){
        ...
        if($ans){
            ...
            $alert = "<div class='alert alert-danger'>Er ging iets fout. </div>";
        }
        die();
    }
